...
// Source File Name:   RPCServiceConfigNode.java
...
public class RPCServiceConfigNode extends ServiceConfigNode
{
	...
	private static final String OPTIONS_NAME = "rpcOptions";
	...
	public RPCServiceConfigNode(byte contents[], int gid, int mid, int sid)
	...
	public RPCServiceConfigNode(String configStr)
		throws AfaConfigException
	{
	...
	protected void initArea()
	{
		...
		setCores(new String[] {
			"identifier", "ip", "port", "ioEventThreads"
		});
		setSwitches(new String[] {
			"registryOptions", "flowControl"
		});
	}

	public Element getElement()
	{
		return getElementByXPath(ConfigFileXPath.lsrServiceOptions(gid, mid, sid, "rpcOptions"));
	}
	...
	private JSONObject createCustomJson(ISchemaNode schemaNode, SchemaManager schema, String parentXPath)
	{
		...
		if (((String)xpathMap.get("registryOptions")).equals(schemaNode.getXPath()))
			json = createCustomRegistryJson(schemaNode, schema, parentXPath);
		return json;
	}

	private JSONObject createCustomRegistryJson(ISchemaNode schemaNode, SchemaManager schema, String parentXPath)
	{
		JSONObject registryJson = JSONObject.fromObject(createCustomItemJson(schemaNode, parentXPath));
		registryJson.remove("value");
		BranchJson eleJson = new BranchJson();
		String xPath = parentXPath != null ? (new StringBuilder(String.valueOf(parentXPath))).append(schemaNode.getXPath()).toString() : schemaNode.getXPath();
		...
		{
			ISchemaNode childNode;
			for (Iterator iterator = xPathOfChilds.iterator(); iterator.hasNext(); childArr.add(createCustomItemJson(childNode, parentXPath)))
			{
				String xPathOfChild = (String)iterator.next();
				childNode = schema.getCustomSchemaNode(xPathOfChild);
			}

		}
		registryJson.put("child", childArr);
		return registryJson;
	}
	...
	public Document createDocumentFromPageJson(String serviceName, JSONObject pageJson)
	{
		...
		Element optionsEle = root.addElement("rpcOptions");
