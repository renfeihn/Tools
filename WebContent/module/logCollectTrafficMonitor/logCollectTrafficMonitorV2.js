	...
	return {
		load:function($el,scope,handler){
			...
			$('#tableContent', $el).on('click', '#performName li', function(event) {
				...
				var id = $(this).index();
				//console.log(id,keyId,statisticstype);
				app.dispatcher.load({
				   title: '应用总览',
			...
			var fakeArr = new Array(15).fill('').map((item,index) => {
				var obj = {
						dataips: "0.00",
						datasize: "0.00",
						ips:[{
						...
						keyid: "10.9.3."+(130+(index+3)),
						keyname: "10.9.3."+(130+(index+3)),
						recorddate: new Date().Format('yyyyMMdd')
				};
				return  obj;
			});
			...
			function getTableData(flag, echartsUrlParamTable, $parentel,url, callback){
				var requestUrl = 'LogStaticsAction_getMonInputByKeyIds.do';
				...
				app.common.ajaxWithAfa({
					...
					if(requestUrl == 'LogStaticsAction_getMonInputByKeyIds.do'){
						data = data.concat(fakeArr);
					}
			...
			function unityData(data,flag,$parentObj,isShowInput){
				...
				for(var key in data){
					//获取到当前所有的系统
					performName.push(data[key].keyname);
				...
				for(var i = 0 ; i < logCollectTMCenter.length ; i ++ ){
					...
					for(var value of logCollectTMData){
						if(value.keyname == dataID){
							setLogData($(logCollectTMCenter[i]),value,i);
							break;
						}
					}
				}
			...
			function setLogData(obj,data,i){
				var $ul = $('.logCollectTM-center>ul:eq('+(i+1)+')',$el);
				var num = $ul.find('.eventBall').length;
				obj.find("span:nth-child(1)").text(data.dataips||'0.00');
				obj.find("span:nth-child(2)").text(data.datasize||'0.00');
				obj.find("span:nth-child(3)").text(0);
				obj.find("span:nth-child(4)").text(num);
				obj.find("span:nth-child(5)").text(0);
			}
			...
			function setProgress(obj,data){
				...
				if(radom>0.95){
					obj.append('<li><span class="'+className+'"><span class="eventBall"></span></span></li>');
				}else{
			...
			$("#performName",$('#tableContent', $el)).on("click","input[type='checkbox']",function(e){
				var keyId = $($(this).parent().siblings()[0]).text();
				if($(this).attr("checked")){
					...
					console.log(index);
					if(index != -1){
						keyID = keyID.slice(0, index).concat(keyID.slice(index + 1, keyID.length));
					}
				}
				if(keyID.length >= 10){
					return;
			...
			function initEventcharts() {
				eventEcharts && eventEcharts.dispose();
				eventEcharts = app.echarts.init($('#eventEcharts', $el)[0]);
				
				var t = $('.span-ePerform-time>span.active',$el).index();
				var s = [8640,4320,720,360,120];
				var len = s[t];

				var xData = new Array(len).fill('').map((item,index) => {
					let time = new Date().getTime() - 5000*index;
					time = new Date(time).Format('hh:mm:ss');
					return time;
				}).reverse();
				...
				var series = [{
					...
					type:'line',
					smooth: true,
					data:new Array(len).fill(0).map(item => {
						if(Math.random() > 0.9){
							return 1;
						}else{
							return 0;
			...
			//////////
			$('.operate-time-btns>span',$el).on('click',function(){
				$(this).addClass('active').siblings().removeClass('active');
			});
			
			
			$el.on('mouseenter','.eventBall',function(e){
				var index = $(this).parents('.logCollectTM-perform').index();
				$('.eventBall-info',$el).find('div:eq(1)>span:eq(1)').text('amlog-0'+index);

				var x = e.pageX - 103;
				var y = e.pageY + 25;
				$('.eventBall-info',$el).removeClass('hide').css({'top':y,'left':x});
			}).on('mouseleave','.eventBall',function(){
				$('.eventBall-info',$el).addClass('hide')
			}).on('click','.eventBall',function(){
