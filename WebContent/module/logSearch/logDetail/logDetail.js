...
//				loadOriginalDetail(scope.serialno, _routing, _index, id, pageInfo);
			...
			function formatEchartsData(logDetailData) {
				...
				for (var i = 0; i < logDetailData.length - 1; i++) {
					...
					if(logDetailData[i]){
						timeEnd = new Date(logDetailData[i]).getTime();
			...
			function drawecharts() {
				if(echartsLine){
					...
					echartsLine = echarts.init($('#logDetailEcharts', $el)[0]);
				}
				var option = {
	...
	//							'serialNo':serialno,
			...
			$('#logSearch', $el).on('click', function(event) {
				...
				baseInfo.forEach(item => {
					if (item.fieldName === '开始时间') {
						searchTime += item.fieldValue
					}
					if (item.fieldName === '结束时间') {
						searchTime += `~` + item.fieldValue
					}
				})
				app.dispatcher.load({
					...
					params:{searchInput:text.toString(), searchTime: searchTime}
				});
			});
			
			let contextP = null;
			$("#logdetailContent", $el).on('contextmenu', function (e) {
				event.preventDefault();
				event.stopPropagation();
				var pageX = e.pageX;
				var pageY = e.pageY;
				$("#message-menu", $el).show().attr('style', `left: ${pageX}px;top: ${pageY}px;z-index: 99999;`);
				$("#message-menu>.dropdown-menu", $el).css('display','block')
				contextP = $(e.target).closest('p');
			})
			
			$(document.body).on('click', function () {
				$("#message-menu", $el).hide()
				$("#message-menu>.dropdown-menu", $el).css('display','none');
			})
			
			initMouseEvent();
			let currentSelect = {};
			function initMouseEvent() {
				$("#logdetailContent", $el).on('mousedown', function (e) {
					event.stopPropagation();
					currentSelect.first = $(e.target).closest('p');
				...
				function mouseup (e) {
					...
					currentSelect.last = $(e.target).closest('p');
					$(document.body).off('mousemove');
					$(document.body).off('mouseup');
					if (!$.isEmptyObject(currentSelect)) {
						var page = parseInt($("#curPage", $el).val()) - 1
						var pageSize = parseInt($("#pageSize", $el).val())
						var startAcqTime = $(currentSelect.first).index() + 1;
						var endAcqTime = $(currentSelect.last).index() + 1;
						startAcqTime = (startAcqTime + page * pageSize)
						endAcqTime = (endAcqTime + page * pageSize)
						if (startAcqTime !== endAcqTime) {
							setSelectAreaToEcharts(startAcqTime, endAcqTime);
						} else {
							echartsLine.dispatchAction({
						        type: 'brush',
						        areas: [
						            {
						                brushType: 'lineX',
			...
			function setSelectAreaToEcharts(startAcqTime, endAcqTime) {
				!endAcqTime && (endAcqTime = startAcqTime);
				!startAcqTime && (startAcqTime = endAcqTime);
				var curPage = parseInt($("#curPage", $el).val());
				var pageSize = parseInt($("#pageSize", $el).val());
//				startAcqTime = startAcqTime + ((startPage || curPage) - 1) * (pageSize);
//				endAcqTime = endAcqTime + ((endPage || curPage) - 1) * (pageSize);
				echartsLine.dispatchAction({
			        type: 'brush',
			        areas: [
			            {
			                brushType: 'lineX',
			                coordRange: [startAcqTime + '', endAcqTime + ''],
			                xAxisIndex: 0
			            }
			        ]
			    });
				let haoshi = echartsData.line1.reduce((sum, item, index) => {
					if (index >= startAcqTime && index <= endAcqTime) {
						return sum += item;
					} else {
						return sum;
					}
				...
				echartsLine.setOption({
					title: {
						text: '共选择' + Math.abs((startAcqTime - endAcqTime)) + '行, 共耗时' + haoshi + 'ms'
					}
				});
			}
			...
			$('#markStart', $el).on('click', function(event) {
				event.preventDefault();
				var text;
				if(document.selection) {
					text = document.selection.createRange();
				...
				var acqtime = $(contextP).index() + 1;
				var page = parseInt($("#curPage", $el).val()) - 1
				var pageSize = parseInt($("#pageSize", $el).val())
				$("#markStartSpan", $el).text((acqtime + page * pageSize) + '，' + text).attr('data-index', (acqtime + page * pageSize))
				contextP = null;
				setEchartsDataSelect()
			});
			
			$('#markend', $el).on('click', function(event) {
				event.preventDefault();
				var text;
				if(document.selection) {
					text = document.selection.createRange();
				...
				var page = parseInt($("#curPage", $el).val()) - 1
				var pageSize = parseInt($("#pageSize", $el).val())
				$("#markendSpan", $el).text((acqtime + page * pageSize) + '，' + text).attr('data-index', (acqtime + page * pageSize))
				contextP = null;
				setEchartsDataSelect()
			});

			function setEchartsDataSelect() {
				var startAcqTime = $("#markStartSpan", $el).attr('data-index');
				var endAcqTime = $("#markendSpan", $el).attr('data-index');
				setSelectAreaToEcharts(parseInt(startAcqTime), parseInt(endAcqTime));
			}
			
			$(".blocktime", $el).on('click', '.fa-times-circle',function (e){
...
//						}
			...
			/* 新折线图接口 */
			loadNewEchartsData(scope.index, scope.id);
			function loadNewEchartsData(index, id) {
				app.common.ajaxWithAfa({
					url : 'ESSearchAction_getOneTradeAcqList.do',
					data:{ index, id }
				}).done(function (data){
					var result = data.result;
					formatEchartsData(result);
					drawecharts()
				})
			}
