	...
	return {
		load : function($el, scope, handler) {			
			var textAreaPosition,
				...
				$logAppSelect = $('[data-role="app_log"]',$el),//日志预警下的应用系统
				$sourceAppSelect = $('[data-role="app_source"]',$el),//关键字预警下的应用系统
				$sourceSelect = $('[data-role="source_select"]',$el),//关键字预警下的数据源
				$keyWordsSelect = $('[data-role="field"]',$el),//关键字预警下的关键字
...
//			        maxDate:new Date(),
		    ...
		    $('[data-role=alertMethod]', $el).click(function(e){
				...
				if(!$this.hasClass('appConfigAlarmStrategy-base-ctn-items-select-hover')){
					...
					$this.addClass('appConfigAlarmStrategy-base-ctn-items-select-hover').siblings().removeClass('appConfigAlarmStrategy-base-ctn-items-select-hover')
					if($this.index()=='0'){
			...
			$el.on('change','[data-role=monitorKpi]',function(){
				var $this = $(this),
					$parentCtn = $this.parents('.appConfigAlarmStrategy-item'),//当前行
				...
				if($this.val()&&$this.val()!='--请选择--'){	
					getKpiList($kpiSubSelect,$this.val());
			...
			this.delegateEvents({
				...
				//选择发送用户
				'click [data-role="sendObj"]':function(){
					var $this = $(this),
					...
					}else if($this.attr('data-id')=='message'){
						$modal.find('h3').text('选择短信发送用户');
					}
					$sendMessageEle = $this.next('.selected-user-list');
...
//					checkResult = checkConditions();
					...
					if(saveFlag){
						...
						allData['app_id'] = app_id || 0;
						allData['obj_id'] = obj_id || 0;
				...
				*/
				...
				'click #appConfigAlarmStrategyUserConfirm':function(){
					...
					if(index == 0 && roleSelectComponent) {
						...
						var userNames = roleSelectComponent.getSelectedValues('role_a');
					...
					let html = '';
					unsaveSendUserList.forEach(item => {
						html += `<span data-id="${item}">${usernameMap[item]}<i class="fa fa-times"></i></span>`;
					});
					$sendMessageEle.html(html);
...
//				$('[data-uuid="'+uuid+'"]', $el).remove();
			...
			function initPage(){
				...
				if(!isModify){
					//默认展示通知策略配置
					$('#alarmStrategyNotice', $el).trigger('click');
				}else {
					//如果是修改模式，则初始化数据
					initModifyData();
				}
				...
				if(scope.sql){
					$('.excute-wrap',$el).removeClass('hide');
					$('#sql_search',$el).val(scope.sql);
				}else{
					$('.excute-wrap',$el).addClass('hide');
				}
			}
			...
			function initCommonVar(){
				app.common.ajaxWithAfa({
					...
					let fakeData = [{name:"${系统名称}"},{name:"${对象名称}"},
									{name:"${设备名称}"},{name:"${指标名称}"},
									{name:"${指标扩展名称}"},{name:"${指标值}"},
									{name:"${对象分类}"},{name:"${首次时间}"},
									{name:"${最后时间}"},{name:"${发生次数}"},
									{name:"${事件类型}"},{name:"${事件级别}"},
									{name:"${阈值}"},{name:"${策略名称}"},
									{name:"${函数名称}"},{name:"${指标分类}"}];
					for(var i=0,html=''; i<fakeData.length; i++){
						var metricKind = fakeData[i];
						html+='<span class="appConfigAlarmStrategy-warn-var-item">'+metricKind.name+'</span>';
					}
					$('.appConfigAlarmStrategy-warn-ctn-var-tit + .appConfigAlarmStrategy-common-var',$el).html(html);
				});
			};
			...
			function initCondition(mid,$condition,conVal){
				...
				app.common.ajaxWithAfa({
					...
					if(data && data.funs){
						...
						if(!$condition){
							$('[data-role="condition"]', $el).html(html);//当前行条件表达式
							$('[data-role="rmeid"]', $el).html(html);//自动恢复表达式
						}else{
							$condition.html(html);
						}
...
//						$table.columns.adjust().draw();
						result.forEach(item => {
							usernameMap[item.userid] = item.role_a;
						});
						if(gid){
			...
			function initRoleSelect() {				
				roleSelectComponent = app.multiSelect({
					...
					isTREnableCheck: false,
	        		context: $el
				});
				$('#appConfigAlarmStrategyRoleTb',$el).off('click').on('click','tbody>tr',function(e){
					e.stopPropagation();
			...
			function initUserSelect() {				
				...
				$('#appConfigAlarmStrategyUserTb',$el).off('click').on('click','tbody>tr',function(e){
					e.stopPropagation();
					$(this).find('input[type="checkbox"]').trigger('click');
				});
			}
			...
			function colletConfigForm(){
				...
				//获取所选择报警配置的数据
				console.log('alertMethod.length',alertMethod.length);
				if(alertMethod.length>0){
					$.each(alertMethod, function(j, mthod){
						...
						if($infoCtn){
							...
							var $triggerTrs = $('.config-item', $infoCtn);
							if(!$triggerTrs||$triggerTrs.length<=0){
								app.alert('请创建：表达式条件！');
								saveFlag = false;
								return;
							}
							console.log('$triggerTrs.length',$triggerTrs.length);
							$.each($triggerTrs, function(index, item){
								var function_index = 1,//行序号
									triggerObj = {};//存报警条件当前行

									itemSource = $('[data-role="kpiSource"]:checked',$(item)).attr('data-id');
									triggerObj['itemSource'] = itemSource;
									if(itemSource == 1){
										triggerObj['app_id'] = $('[data-role="app_log"]',$(item)).val();
										triggerObj['busiField'] = $('[data-role="busi_field"]',$(item)).val();
										triggerObj['busiItem'] = $('[data-role="busi_item"]',$(item)).val();
										triggerObj['mid'] = $('[data-role="busi_item"]>option:selected',$(item)).attr('data-id');
										app_id = triggerObj['app_id'];
									}else if(itemSource == 2){
										triggerObj['app_id'] = $('[data-role="app_source"]',$(item)).val();
										triggerObj['keySourceid'] = $('[data-role="source_select"]',$(item)).val();
										triggerObj['keyField'] = $('[data-role="field"]',$(item)).val();
										app_id = triggerObj['app_id'];
									}else if(itemSource == 3){
										triggerObj['mid'] = $('[data-role="kpiSub"]',$(item)).val();
									}

									triggerObj['symbol'] = $('[data-role="condition"]',$(item)).val();
									triggerObj['threshold'] = $('[data-role="threshold"]',$(item)).val();
									triggerObj['rmeid'] = $('[data-role="rmeid"]',$(item)).val();
									triggerObj['recover_value'] = $('[data-role="recoverValue"]',$(item)).val();
...
//							$('[data-role="expression"]:checked', $infoCtn).index();
							...
							let upgrade_type = $('[data-role="upgrade"]:checked', $infoCtn).attr('data-id');
							...
							config_trigger['upgrade_type'] = parseInt(upgrade_type);
							if(upgrade_type != 0){
								config_trigger['upgrade_range'] = $('[data-role="upgrade_range"]',$infoCtn).val();
							}

							if(scope.sql){
								//另存为预警
								config_trigger['busiPlanType'] = $('#planType',$infoCtn).val();
								if(config_trigger['busiPlanType'] == 1){
									config_trigger['busiScheduleType'] = $('#scheduleType',$infoCtn).val();
									config_trigger['busiIntervalTime'] = '';
								}
							}
							...
							// configNoticeObj['notice_send_type'] = 0;

							// var userlist = [];
							// $('.selected-user-list>span',$el).each((index,item) => {
							// 	let type = $(item).parent().attr('data-id');
							// 	userlist.push(type + '_' + $(item).attr('data-id'));
							// });
							// configNoticeObj['notice_user_list'] = userlist.join(',');
							
							$('#message-wrap .appConfigAlarmStrategy-msgConfig-ctn',$el).each((index,item) => {
								
								$('[data-role="sendObj"]',$(item)).each((ind,it) => {
									if($(it).attr('data-senduser')){
										let notice_send_type = $(it).next().attr('data-id');
										let configNoticeObj = {
											notice_type: index,
											notice_send_type: notice_send_type,
											notice_user_list: $(it).attr('data-senduser')
										};
			...
			function colletInfoForm(){
				var alertMessage = $('#appConfigAlarmStrategyWarnMsg', $el).val(),//报警信息
					...
					resoleAdvise = $('#appConfigAlarmStrategyAdvice', $el).val(),//升级信息
					jsonObj = {};
				...
				if(alterId){
					app.common.ajaxWithAfa({
						...
						funs.forEach(function(fun,index){
							...
							getTriggerByalter_id(trigger_id,event_type,fun);
						});
					});
				}
			}
			...
			function getTriggerByalter_id(trigger_id,event_type,fun){
				...
				//公共信息
				$('[data-role="kpiSource"][data-id="'+fun['item_source']+'"]',$curCtn).prop('checked',true).trigger('change');	//指标来源
				setTimeout(() => {
					if(fun['item_source'] == 1){
						$('[data-role="app_log"]',$curCtn).next().remove();
						$('[data-role="app_log"]',$curCtn).val(fun['app_id']).tinyselect();;	//业务预警下的应用系统
					}else if(fun['item_source'] == 2){
						$('[data-role="app_source"]',$curCtn).next().remove();
						$('[data-role="app_source"]',$curCtn).val(fun['app_id']).tinyselect();	//关键字预警下的应用系统
						getdataSource(fun['app_id']).then(res => {$sourceSelect.val(fun['key_sourceid'])});
						getKeyWords(fun['key_sourceid']).then(res => {$keyWordsSelect.val(fun['key_field'])});
					}

					$('[data-role="busi_field"]',$curCtn).val(fun['busi_field']);	//统计维度
					$('[data-role="busi_item"]',$curCtn).val(fun['busi_item']);		//指标列表
					
					$('.appConfigAlarmStrategy-upgrade-radio>span:eq('+fun['upgrade_type']+')>input',$curCtn).prop('checked',true).trigger('change'); //升级策略
					$('[data-role="upgrade_range"]',$curCtn).val(fun['upgrade_range']);	//发生时间、次数
				},1000);
				...
				app.common.ajaxWithAfa({
					...
					var private_rsp = data.funs[0];
					var alert_times = private_rsp.alertValue == 'null' ? 0 : private_rsp.alertValue,//连续触发次数
...
//					$aOr.val($.trim(join_exp).toLocaleLowerCase()+'');//与或表达式
			...
			function updateVarCtn($kpiSub){
				let $p =  $kpiSub.parents('.appConfigAlarmStrategy-item');
				var $monitorKpi =$('[data-role="monitorKpi"]',$p),
			...
			function updateKpiSub(mid){
				var $kpiSub = $('[data-role=kpiSub]', $('.kpi-selector:not(.hide)', $el)),
					varItemClass = 'appConfigAlarmStrategy-warn-var',
					$kpiSel = $('[data-role=monitorKpi]', $('.kpi-selector:not(.hide)', $el));
					
				//清空常用变量
				$kpiVarCtn.empty();
				
				$.each($kpiSub, function(index, sub){
					let $p = $(sub).parents('.appConfigAlarmStrategy-item');
					var kpiId = $('[data-role="monitorKpi"]',$p).val(), //指标值
						$monitorKpi = $(sub).parent().prev().find('select'),//指标
						$con = $('[data-role="condition"]',$p),//条件表达式
						$rmeid = $('[data-role="rmeid"]',$p),//自动恢复表达式
						subVal = $(sub).val(),//指标项值
			...
			function bindEvents() {
				//选择kpi来源
				$('.appConfigAlarmStrategy-item',$el).on('change','input[data-role="kpiSource"]',function(){
					let $root = $(this).parents('.appConfigAlarmStrategy-item');
					let index = $('input[data-role="kpiSource"]:checked',$root).index();
					$('.kpi-selector-wrap>div:eq('+index+')',$root).removeClass('hide').siblings().addClass('hide');
					//$('.appConfigAlarmStrategy-common-var,.appConfigAlarmStrategy-warn-ctn-var-ctn',$el).empty();
					//$('.appConfigAlarmStrategy-msgConfig-textarea',$el).val('');
					$('.appConfigAlarmStrategy-warn-ctn-var-ctn',$el).empty();
					if($(this).attr('data-id') == 1){
						$('.appConfigAlarmStrategy-warn-ctn-var-ctn',$el).html(`
						...
						$('.appConfigAlarmStrategy-warn-ctn-var-ctn',$el).html(`
						<div class="appConfigAlarmStrategy-common-var">
							<span class="appConfigAlarmStrategy-warn-var-item" title="数据源">$ {数据源}</span>
							<span class="appConfigAlarmStrategy-warn-var-item" title="字段">$ {字段}</span>
						</div>
						`);
					}
				...
				//选择升级策略
				$('.appConfigAlarmStrategy-item',$el).on('change','input[data-role="upgrade"]',function(){
					let $root = $(this).parents('.appConfigAlarmStrategy-item');
					let index = $('input[data-role="upgrade"]:checked',$root).parent().index();
					index == 0 && $('.upgrade-kpi',$root).addClass('hide');
					index == 1 && $('.upgrade-kpi',$root).removeClass('hide') &&  $('.upgrade-kpi>.upgrade-kpi-label',$root).text('发生次数');
					index == 2 && $('.upgrade-kpi',$root).removeClass('hide') &&  $('.upgrade-kpi>.upgrade-kpi-label',$root).text('持续时间(分钟)');
				});

				//选择应用系统后筛选日志源
				$('.appConfigAlarmStrategy-item',$el).on('change','[data-role="app_source"]',function(){
					let id = $(this).val();
					getdataSource(id);
				});

				//选择日志源后查询关键字
				$('.appConfigAlarmStrategy-item',$el).on('change','[data-role="source_select"]',function(){
					let id = $(this).val();
					getKeyWords(id);
				});

				//删除已选发送用户
				$('.selected-user-list',$el).on('click','>span>i',function(){
					let $p = $(this).parents('.selected-user-list');
					let $r = $(this).parents('.message-send-wrap');
					$(this).parent().remove();
					let sendusers = Array.from($p.children()).map(item => $(item).attr('data-id')).join(',');
					$p.prev().attr('data-senduser',sendusers);
					if($r.attr('data-role') == 'bj') {
						synSendList();
					}
				});

				//执行计划
				$('.appConfigAlarmStrategy-item',$el).on('change', 'select[name="scheduleType"]', function(event) {
					var value = $(this).val();
					var $p = $(this).parents('.appConfigAlarmStrategy-item');
					$(this).parent().parent().next().children().addClass('hide');
					if(value == 1){
						$('#minute', $p).removeClass('hide');
					}else if(value == 2){
						$('#time', $p).removeClass('hide');
					}else if(value == 3){
						$('#day', $p).removeClass('hide');
						$('#time', $p).removeClass('hide');
					}else if(value == 4){
						$('#date', $p).removeClass('hide');
						$('#time', $p).removeClass('hide');
					}
				});

				//计划还是实时
				$('#planType',$el).on('change',function(){
					let val = $(this).val();
					val == 1 ? $('#planCondition',$el).removeClass('hide') : $('#planCondition',$el).addClass('hide');
				});
			}
			
			function getKpiList($kpiSubSelect,metricKind,val) {
				app.common.ajaxWithAfa({
					"url": "MonitorCfgManagerAction_queryAllMetricItem.do",
					"data": {
					...
					if(data && data.metricItems){
						...
						let html = '';
						for(var i=0; i<metricKinds.length; i++){
							html += '<option data-item="'+metricKinds[i]['item']+'" data-mitId="'+metricKinds[i]['id']+'" value="'+metricKinds[i]['id']+'">'+metricKinds[i]['display_name']+'</option>';
						}
						$kpiSubSelect.empty().append('<option>--请选择--</option>');
						$kpiSubSelect.siblings('.tinyselect').remove();
						$kpiSubSelect.append(html);
						val && $kpiSubSelect.val(val);
			...
			function getApps() {
				app.common.ajaxWithAfa({
					...
					let html = '';
					content.cate1Objects.forEach(item => {
						let data = item.objectSummary;
						html += `<option value="${data.objectId}">${data.objectName}</option>`;
					});
					$sourceAppSelect.empty().append('<option value="0">--请选择--</option>');
					$logAppSelect.empty().append('<option value="0">--请选择--</option>');
					$sourceAppSelect.append(html);
					$logAppSelect.append(html);
					$sourceAppSelect.tinyselect();
					$logAppSelect.tinyselect();
				});
			}
			...
			function getdataSource(appId) {
				return new Promise(resolve => {
					app.common.ajaxWithAfa({
						...
						$sourceSelect.empty().append('<option value="0">--请选择--</option>');
						$sourceSelect.siblings('.tinyselect').remove();
						$sourceSelect.append(html);
						$sourceSelect.tinyselect();
						resolve(content);
					});
				})
			...
			//查数据源对应的关键字
			function getKeyWords(sourceId) {
				return new Promise(resolve => {
					app.common.ajaxWithAfa({
						url: 'LogCfgSourceAction_getSourceField.do',
						data: {
							sourceId: sourceId
						}
					}).done(content => {
						let html = '';
						content.result.forEach(item => {
							html += `<option value="${item.fieldKey}">${item.fieldDesc}</option>`;
						});
						$keyWordsSelect.empty().append('<option value="0">--请选择--</option>');
						$keyWordsSelect.siblings('.tinyselect').remove();
						$keyWordsSelect.append(html);
						$keyWordsSelect.tinyselect();
						resolve(content);
					});
				})
			...
			//报警发送人同步至恢复发送人
			function synSendList() {
				let $alertWrap = $('#message-wrap .appConfigAlarmStrategy-msgConfig-ctn:eq(0)',$el);
				let $recoverWrap = $('#message-wrap .appConfigAlarmStrategy-msgConfig-ctn:eq(2)',$el);
				let list = [],html = '';
				$('.selected-user-list',$alertWrap).children().each((index,item) => {
					let id = $(item).attr('data-id');
					let name = $(item).text();
					list.push(id);
					html += `<span data-id="${id}">${name}<i class="fa fa-times"></i></span>`;
				});
				$('.selected-user-list[data-id="1"]',$recoverWrap).html(html);
				$('.selected-user-list[data-id="1"]',$recoverWrap).prev().attr('data-senduser',list.join(','));
