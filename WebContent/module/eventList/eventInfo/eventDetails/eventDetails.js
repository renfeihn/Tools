define([ "jquery" ], function() {
	...
	return {
		load : function($el, scope, handler) {
			...
			$('#disposeModal',$el).modal('show').modal('hide');
			// 解决按钮事件
					...
					success : function(data){
						if(data.status){							
							if(data.content && data.content.eventInfoDsp){
								...
								// 处理两个按钮是否可用
								if(data.eventStatus === 0 && (data.itilStatus === '' || data.itilStatus === '待分派')) {
								...
								$('#eventSource', $el).text(getEventSourceName(data.eventSource));
								...
								$('#stayTime', $el).text(data.eventDuration).attr("title",data.eventDuration);//持续时间
								...
								if(data.upgradeReason){
									$("#addEventInfo",$el).attr('title', data.upgradeReason).text(data.upgradeReason);
								}else{
									$("#addEventInfo",$el).parent().css("display","none");
								}
								if(data.dealMsg){//事件解除
									var messObj =  JSON.parse(data.dealMsg);
								...
								/***************暂无数据******************/
								$('#eventDecribe', $el).attr('title', data.eventDesc).text(data.eventDesc);//事件描述
								...
								app.domain.clearScope("eventDetailToRelative");
								app.domain.exports("eventDetailToRelative", obj);
							}							
						}
					}
				});
			...
			function getEventSourceName(data) {
				switch(data){
					case '00':
						return 'aim';
					case '01':
					...
					if(data.status){						
						...
						if(ips && ips.length){
							...
							for( var i = 0; i < ips.length; i++ ) {
								if(ips[i].ip){
									$('#deployIP', $el).text(ips[i].ip);//部署IP
									break;
								}
							}	
							$('#ipSelt', $el).html(optionTemp);
							
							getMetrics(ips[i].objId);
						}						
					}
				}
			});
			...
			//ip下拉框
			$('#ipSelt', $el).change(function(e){
				getMetrics( $(this).val() );
			});
			
			//获取模态框指标下拉框数据
			function getMetrics(objectId){
				$('#itemSelt', $el).empty();
				$.ajax({
					type: "post",
			...
			$('#deployIP', $el).click(function(){
				var val = $.trim($(this).text());
				app.common.ajaxWithAfa({
					url:'CommonMonitorAction_getObjectByIp.do',
					data:{
						"ip":val
					}
				}).done(function(data){
					...
					//详情链接映射
					var detailLinkMap = {
						linux: 'basicMonitor#operatingSystem#linuxSummary#LinuxTabs',
						aix: 'basicMonitor#operatingSystem#aixSummary#aixTabs',
						windows: 'basicMonitor#operatingSystem#windowsSummary#windowsTabs',
						hpux: 'basicMonitor#operatingSystem#hpuxSummary#hpuxTabs'
					}
					var section = detailLinkMap[result.l3CateName].split('#');
					
					app.dispatcher.load({
						title: result.l3CateName + "详情-" + result.serviceIp,
						moduleId: section.shift(),
						section: section,
						id:"",
						params:{
							objectId: result.objectId,
							category: result.l3CateName,
							hostname: result.serverName,
							ipadd: result.serviceIp,
			...
			$('#itemModal [data-role=confirm]', $el).click(function(){//指标模态框中的确认按钮
				...
				var name = $('#itemSelt', $el).val().split('@')[0];
				var unit = $('#itemSelt', $el).val().split('@')[1];
				var objectId = $('#itemSelt', $el).val().split('@')[2];
						...
						if(data && data.length){
							...
							data.forEach(function(item, index){				
								liTemp += '<li name="'+item.name+'" time="'+item.time+'" value="'+item.value+'" unit="'+item.unit+'">\
										        <a data-toggle="tab" class="eventDetails-tabs-a" title="'+ item.displayName +'">'+item.displayName+'</a>\
										   </li>';						
							});
