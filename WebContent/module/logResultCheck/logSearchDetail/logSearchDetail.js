...
//    					}, 200)
    			...
    			'aoColumnDefs': [{
    				'render':function(data,type,row,meta){
    					return '<i title="日志原文分析" class="fa fa-file-text-o"></i><i title="日志文件查看" style="margin-left: 6px;" class="fa fa-files-o"></i><i title="智能链路分析" class="fa fa-bar-chart" style="margin-left: 6px;"></i><i title="精准链路分析" class="fa Example of podcast fa-podcast" style="margin-left: 6px;"></i>'+
    							'<i title="下载" class="fa fa-download" style="margin-left: 6px;"></i><i title="分享" class="fa fa-share-alt" style="margin-left: 6px;"></i>'+
    							'<div class="log-Down"><i class="fa fa-file-word-o" title="doc"></i><i class="fa fa-file-text" style="margin-left: 6px;" title="txt"></i></div>'+
...
//    								data[i].LOG = setSelect(data[i].LOG);
    	...
    	$("#modalShow", $el).on("click", function (e) {
    		...
    		switch(id) {
	    		case 0:
	    			$(this).attr('data-id', '1').text('切换至精简模式');
	    			showOther()
	    			break;
	    		case 1:
	    			$(this).attr('data-id', '0').text('切换至普通模式');
	    			closeOther()
	    			break;
    		}
    	})
...
//			var PARENT = lastItem.attr('data-parent');
		...
		// 文件查看
		.on('click', '.fa-files-o', function(event) {
			var tr = $logTable.row($(this).parent().parent()).data();
			var ACQTIME = null;
			try {
				ACQTIME = tr.__context__[0].ACQTIME - 1
			} catch (e) {
			}
			var searchInput = $('#searchInput', $el).html().trim();
			$(this).css('color','#541b8a');
			...
			tr['_head_.logsn'] = logsn;
			app.dispatcher.load({
				"title": "日志文件明细 - "+no.substring(0, 4)+'...'+no.substr(-4, 4),
