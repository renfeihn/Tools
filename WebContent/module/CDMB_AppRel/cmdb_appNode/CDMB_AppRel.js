			...
			function svgAddAnimation() {
				 $('.svg-wrap svg>g>g',$el).each(function() {
				     ...
				     if (_class == 'edge') { // edge
				         ...
				         if (!edgesTargetMap[target]) {
				        	 edgesTargetMap[target] = [];
				         }
				         edgesTargetMap[target].push(edge);
				         ...
				         if (id.indexOf('_category') == -1) { // category
				             ...
				             $($this.find('path')).attr('stroke', '#f2f2f2').attr('fill', 'transparent');
				             nodeIdMap[id] = node;
				         }
				     }
				 });
				 ...
				 keys.forEach((key, index) => {
					 ...
					 nodeList.forEach((node, index) => {
						 let nodeId = node.id;
						 /* 找到节点上的连线 */
						 let edgeList = edges[nodeId];
						 let tmp = {node};
						 if (edgeList && edgeList.length) {
							 tmp = Object.assign({}, tmp, {edgeList});
						 }
						 let path = resionous(node).split(',').reverse();
						 tmp.path = path;
						 finalArr.push(tmp);
					 })
				 })
				 ...
				 /* 加载每个节点的其他信息 */
				 function resionous(node) {
					 var id = node.id;
					 if (!edgesTargetMap[id]) {
						 return id;
					 }
					 let pid = edgesTargetMap[id][0].src;
					 let no = nodeIdMap[pid];
					 return node.id + ',' + resionous(no);
				 }
				 
				// 数组排序算法
				function finalArrSort(a, b) {
					var src_subx = parseInt(a.node.x, 10) - parseInt(b.node.x, 10),
						src_suby = parseInt(a.node.y, 10) - parseInt(b.node.y, 10);
					if (src_subx !== 0) {
						return src_subx;
					} else {
						return src_suby;
				...
				finalArr.sort(finalArrSort);
				 ...
				 function newChange(findItem) {
					 var node = findItem.node;
					 var edgeList = findItem.edgeList;
					 var gItem = node.$el;
					 $($el.find('path')).attr('filter','');
					 $(gItem.find('path')).attr('fill', 'rgb(207, 232, 252)').attr('stroke', 'rgb(65, 105, 225)')
					 return setInterval(function() {
						 edgeList && edgeList.forEach(item => {
							 $('#' + item.id)
							 .find("path")
							 .attr('stroke-width', 5)
							 .attr('stroke-dasharray', '0.1,8')
				 ...
				 function newChangeColor(findItem) {
					 var color = getRodomColor();
					 var edgeList = findItem.edgeList;
					 $($el.find('path')).attr('filter','');
					 edgeList && edgeList.forEach(item => {
				 ...
				 function iterator(interval) {
					finished = false;
					 ...
					 if (index < len) {
						 ...
						 interval = newChange(finalArr[index]);
					 } else {
						 clearInterval(interval);
						 finished = true;
						 return;
					 ...
					 setTimeout(function() {
						 let path = finalArr[index].path;
						 let node = finalArr[index].node;
						 let p = getParamsNew(path, node);
						 
						 Promise.all([p]).then(res => {
							 newChangeColor(finalArr[index]);
							 index++;
							 iterator(interval);
						 });
...
//				$('.svg-wrap>svg>g',$el).attr('transform','scale(0.7) rotate(0) translate(-80,420)');
			...
			function getTopo() {
				app.common.ajaxWithAfa({
					url:'CmdbConfigManagerAction_getAppSourceSvgInfo.do',
					data:{
						appId: scope.params.id,
					...
					if(data.result){
						...
						getEvents().then(res => {
							...
							setTimeout(function () {
								var filter = setShadow();
								$('.svg-wrap',$el).find('svg').append(filter);
								setSvg();
								setEvents();
								svgAddAnimation();
								addEventSvg();
			...
			function setShadow() {
				return `<filter id="inset-shadow" x="-50%" y="-50%" width="200%" height="200%">
			...
			function getParams(path) {
				let sqlSearch = '*|select count(*) as count, avg(duration) as avgDuration from applog-* where ';
				path = path.forEach((item, index) => {
					index === 0 && unitySql('appid',item);
					index === 1 && unitySql('objectid',item);
					index === 2 && unitySql('hostip',item);
					index === 3 && unitySql('sourceid',item);
				})
				
				function unitySql(key, value) {
					sqlSearch += `_head_.${key}="${value}" and `;
				}
				...
				let params = {
					...
					cate: JSON.stringify({
						...
						"app": {
							"cate1": [{cateId: scope.params.cate1}],
							"cate2": [{cateId: scope.params.cate2}],
							"cate3": [{cateId: scope.params.id}]
						}
					}),
					logType: 1
				};
				return Object.assign({}, {search: sqlSearch}, params);
			}
			
			function getParamsNew (path, node) {
				let p = getParams(path);
				return sqlSearch(p, node);
			}
			

			function sqlSearch(urlData, node) {
    			return new Promise(resolve => {
					app.common.ajaxWithAfa({
						...
						node = node.$el;
						let avg = obj.avgDuration == 'NaN' ? 0 : obj.avgDuration;
						if(avg > 100){
