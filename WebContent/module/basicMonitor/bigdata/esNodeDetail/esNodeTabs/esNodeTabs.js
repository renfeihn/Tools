define(["jquery"],function(){
	...
	return {
		...
		load:function($el,scope,handler){
			...
			$('#tabApp', $el).on('click', 'li', function() {
				...
				if (!$this.hasClass('active')) {
					...
					cache.push(path + scope.objectId);
					...
					app.dispatcher2.load({
						"title" : title + scope.objectId,
						"moduleId" : args[0],
						"section" : args.slice(1),
						"frameRenderTo" : '#view',
						"id" : path + scope.objectId,
						"params" : scope,
						"context" : $el
					});
		...
		resume:function($el,scope,handler){
			app.dispatcher2.resume($('section li.active' ,$el).attr('data-address') + scope.objectId);
		}
