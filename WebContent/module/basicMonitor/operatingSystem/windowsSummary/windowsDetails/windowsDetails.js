	...
	return {
		load : function($el, scope, handler) {
			...
			app.common.ajaxWithAfa({
				url:"LinuxMonitorAction_getProcessList.do",
			...
			app.common.ajaxWithAfa({
				...
				$(".windowsDetails-totalProgress",$el).html(data.result||'-');
			});
			//僵尸进程数
			app.common.ajaxWithAfa({
				url:"CommonMonitorAction_getMetricCount.do",
				...
				$(".windowsDetails-zombieProgress",$el).html(data.result||'-');
			});
				...
				cpuEchart = app.drawEcharts($.extend({}, basicEcharts, {
					...
					succfn: function(data){
						if(data.content.echartsData && data.content.echartsData.line1 && data.content.echartsData.line1.length){
							...
							$('#system', $el).text(data.line3[data.line3.length-1] + '%');
							$('#user', $el).text(data.line2[data.line2.length-1] + '%');
							$('#waiting', $el).text(data.line1[data.line1.length-1] + '%');
						}	
					}
				}));
				cpuEchart.start();
					...
					if(i == 0){
						...
						divTemp += '<div id="'+data[i]+'" class="tab-pane active">\
										<div class="windowsDetails-layout">\
											<div id="'+(data[i] + 'Echart')+'" style="width: calc(100% - 122px);height: 214px;"></div>\
											<div style="width: 102px;margin-left: 20px;flex: none">\
												<span class="windowsDetails-netIn">-</span>\
												<span class="windowsDetails-netOut">-</span>\
											</div>\
										</div>\
									</div>';	
					}else{
				...
				netCardEchart = app.showEcharts({//网卡
					...
					succfn: function(res){
						if(res && res.content && res.content.echartsData && res.content.echartsData.line1 && res.content.echartsData.line1.length){
							...
							$('#' + data[0], $el).find('.windowsDetails-netIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
							$('#' + data[0], $el).find('.windowsDetails-netOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
						}
					}
				});
				netCardEchart.start();
						...
						netCardEchart = app.showEcharts({//网卡
							...
							succfn: function(data){
								if(data && data.content && data.content.echartsData && data.content.echartsData.line1 && data.content.echartsData.line1.length){
									...
									$('#' + netCard, $el).find('.windowsDetails-netIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
									$('#' + netCard, $el).find('.windowsDetails-netOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
								}
							}
						});
						netCardEchart.start();
			...
			function showDiskCtn(data){
				...
				for(var i in data){
					if(i == 0){
						liTemp += '<li class="active"><a href="#tabs'+i+'" data-toggle="tab">'+data[i]+'</a></li>';
						divTemp += '<div id="tabs'+i+'" class="tab-pane active">\
										<div class="windowsDetails-layout">\
											<div id="'+('diskEchart'+i)+'" style="width: 100%;height: 224px;"></div>\
										</div>\
									</div>';	
					}else{
						liTemp += '<li><a href="#tabs'+i+'" data-toggle="tab">'+data[i]+'</a></li>';
						divTemp += '<div id="tabs'+i+'" class="tab-pane">\
										<div class="windowsDetails-layout">\
											<div id="'+('diskEchart'+i)+'" style="width: 100%;height: 224px;"></div>\
										</div>\
									</div>';	
				...
				dickEchart = app.drawEcharts({//硬盘
					...
					/*succfn: function(res){
						if(res && res.content && res.content.echartsData && res.content.echartsData.line1 && res.content.echartsData.line1.length){
							var eData = res.content.echartsData;
							$('#tabs0', $el).find('.windowsDetails-diskIO').text(eData.line3[eData.line3.length - 1] + '%');
							$('#tabs0', $el).find('.windowsDetails-diskIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
							$('#tabs0', $el).find('.windowsDetails-diskOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
						}
					}*/
				});
				dickEchart.start();	
			...
			$('#diskTab', $el).on('click', 'li', function(){
				if($(this).hasClass('active')){return}
				var $index = $("#diskTab li",$el).index(this);
					...
					if($('#' + disk, $el).hasClass('active')){
						...
						dickEchart = app.drawEcharts({//硬盘
							...
							/*succfn: function(data){
								if(data && data.content && data.content.echartsData && data.content.echartsData.line1 && data.content.echartsData.line1.length){
									var eData = data.content.echartsData;
									$('#' + disk, $el).find('.windowsDetails-diskIO').text(eData.line3[eData.line3.length - 1] + '%');
									$('#' + disk, $el).find('.windowsDetails-diskIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
									$('#' + disk, $el).find('.windowsDetails-diskOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
								}
							}*/
						});
						dickEchart.start();
