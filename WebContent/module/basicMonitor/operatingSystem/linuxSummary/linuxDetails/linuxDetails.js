	...
	return {
		load: function ($el, scope, handler) {
			...
			function getData() {
				app.common.ajaxWithAfa({
					...
					if (!$.isEmptyObject(data.result)) {
						$('#cpu', $el).text(data.result['os.cpu.pcpu'].value !== undefined ? Number(data.result['os.cpu.pcpu'].value).toFixed(2) : '-'); //cpu
						$('#nc', $el).text(data.result['os.mem.mem_used_pct'].value !== undefined ? Number(data.result['os.mem.mem_used_pct'].value).toFixed(2) : '-'); //内存使用率 
						$('#disk', $el).text(data.result['os.disk.busy_pct'].value !== undefined ? Number(data.result['os.disk.busy_pct'].value).toFixed(2) : '-'); //磁盘使用率 
						$('#net', $el).text(data.result['os.net.tkps'].value !== undefined ? Number(data.result['os.net.tkps'].value).toFixed(2) : '-'); //网络流量	
					}
				});
			}
			/*组织折线图数据*/
			...
			function showDiskCtn(data) {
				...
				var divTemp = "";
				for (var i in data) {
					let name = data[i].replace('/', 'R_');
					if (i == 0) {
						liTemp += '<li class="active"><a href="#' + name + '" data-toggle="tab">' + name + '</a></li>';
						divTemp += '<div id="' + name + '" class="tab-pane active">\
										<div class="linuxDetails-layout">\
											<div id="' + (name + 'Echart') + '" style="width: calc(100% - 122px);height: 224px;"></div>\
											<div style="width: 102px;margin-left: 20px;flex: none">\
						...
						liTemp += '<li><a href="#' + name + '" data-toggle="tab">' + name + '</a></li>';
						divTemp += '<div id="' + name + '" class="tab-pane">\
										<div class="linuxDetails-layout">\
											<div id="' + (name + 'Echart') + '" style="width: calc(100% - 122px);height: 224px;"></div>\
											<div style="width: 102px;margin-left: 20px;flex: none">\
												<span class="linuxDetails-diskIO">-</span>\
				...
				let name0 = data[0].replace('/', 'R_');
				dickEchart = app.drawEcharts({ //硬盘
					handler: handler,
					context: $el,
					selector: '#' + name0 + 'Echart',
					eType: 'line',
					url: 'CommonMonitorAction_getEachart2.do',
					unit: 'kbps',
					items: ['in', 'out'],
					...
					succfn: function (res) {
						if (res && res.content && res.content.echartsData && res.content.echartsData.line1 && res.content.echartsData.line1.length) {
							var eData = res.content.echartsData;
							$('#' + name0, $el).find('.linuxDetails-diskIO').text(eData.line3[eData.line3.length - 1] + '%');
							$('#' + name0, $el).find('.linuxDetails-diskIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
							$('#' + name0, $el).find('.linuxDetails-diskOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
						}
					}
				});
				dickEchart.start();
			...
			app.common.ajaxWithAfa({
				...
				$('#version', $el).text(data.version == undefined ? '-' : data.version);
				$('#ip', $el).text(data.ipAdd == undefined ? '-' : data.ipAdd);
				$('#port', $el).text(data.hostName == undefined ? '-' : data.hostName);
				// 显示更多信息
				$('#moreInfo', $el).popover({
					'placement': 'bottom',
					'title': '详细信息',
