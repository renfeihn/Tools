	...
	return {
		load : function($el, scope, handler) {
			...
			app.common.ajaxWithAfa({
				...
				$(".aixDetails-totalProgress",$el).html(data.result||'-');
			});
			...
			app.common.ajaxWithAfa({
				...
				$(".aixDetails-zombieProgress",$el).html(data.result||'-');
			});
			...
			function getData(){
				app.common.ajaxWithAfa({
					...
					data:{
						"objectId":objectId,
						'metricNames':['aix_cpu.used_pct','aix_mem.used_pct','aix_disk.util_pct','aix_net.rtxkB']
					}
				}).done(function (data) {
					if(!$.isEmptyObject(data.result)){
						$('#cpu', $el).text(data.result['aix_cpu.used_pct'].value !== undefined?data.result['aix_cpu.used_pct'].value.toFixed(2):'-');//cpu
						$('#nc', $el).text(data.result['aix_mem.used_pct'].value !== undefined?data.result['aix_mem.used_pct'].value.toFixed(2):'-');//内存使用率 
						$('#disk', $el).text(data.result['aix_disk.util_pct'].value !== undefined?data.result['aix_disk.util_pct'].value.toFixed(2) :'-');//磁盘使用率 
						$('#net', $el).text(data.result['aix_net.rtxkB'].value !== undefined?data.result['aix_net.rtxkB'].value.toFixed(2):'-');//网络流量	
					}
				});	
			}
			/*组织折线图数据*/
			...
			function fnCpuEchart(){
				cpuEchart = app.drawEcharts($.extend({}, basicEcharts, {
					...
					urlParams: $.extend({}, urlParams, {
						metricNames : ['aix_cpu.system_pct','aix_cpu.user_pct','aix_cpu.iowait_pct']
					}),
			...
			function getNeicunData(){
				...
				app.common.ajaxWithAfa({
					...
					data:{
						"objectId":objectId,
						'metricNames':['aix_mem.used','aix_mem.free','aix_swap.used','aix_swap.free']
					}
				}).done(function (data) {
					if(!$.isEmptyObject(data.result)){
						memUsed = (data.result['aix_mem.used'].value/1024).toFixed(2);
						memFree = (data.result['aix_mem.free'].value/1024).toFixed(2);
						xnmemUsed = (data.result['aix_swap.used'].value/1024).toFixed(2);
						xnmemFree = (data.result['aix_swap.free'].value/1024).toFixed(2);
			...
			function showNetCtn(data){
						...
						divTemp += '<div id="'+data[i]+'" class="tab-pane active">\
										<div class="aixDetails-layout">\
											...
											<div style="width: 102px;margin-left: 20px;flex: none">\
												<span class="aixDetails-netIn">-</span>\
												<span class="aixDetails-netOut">-</span>\
											</div>\
										</div>\
									</div>';	
					}else{
				...
				netCardEchart = app.showEcharts({//网卡
					...
					urlParams: {
						...
						objectId: objectId,
						tagvs: data[0],
						metricNames:['aix_net.rxpck','aix_net.txpck']
					},
					beforefn:function (data) {
						data = formatLineData(data);
					},
					succfn: function(res){
						if(res && res.content && res.content.echartsData && res.content.echartsData.line1 && res.content.echartsData.line1.length){
							...
							$('#' + data[0], $el).find('.aixDetails-netIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
							$('#' + data[0], $el).find('.aixDetails-netOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
						}
					}
				});
				netCardEchart.start();
						...
						netCardEchart = app.showEcharts({//网卡
							...
							succfn: function(data){
								if(data && data.content && data.content.echartsData && data.content.echartsData.line1 && data.content.echartsData.line1.length){
									...
									$('#' + netCard, $el).find('.aixDetails-netIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
									$('#' + netCard, $el).find('.aixDetails-netOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
								}
							}
						});
						netCardEchart.start();
			...
			function showDiskCtn(data){
				...
				dickEchart = app.drawEcharts({//硬盘
					...
					urlParams: {
						...
						objectId: objectId,
						tagvs: data[0],
						metricNames : ['aix_disk.read_ps','aix_disk.written_ps','aix_disk.util_pct']
					},
					beforefn: function(data){
						data = formatLineData(data);
					},
					succfn: function(res){
						if(res && res.content && res.content.echartsData && res.content.echartsData.line1 && res.content.echartsData.line1.length){
							var eData = res.content.echartsData;
							$('#' + data[0], $el).find('.aixDetails-diskIO').text(eData.line3[eData.line3.length - 1] + '%');
							$('#' + data[0], $el).find('.aixDetails-diskIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
							$('#' + data[0], $el).find('.aixDetails-diskOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
						}
					}
				});
				dickEchart.start();	
					...
					if($('#' + disk, $el).hasClass('active')){
						...
						dickEchart = app.drawEcharts({//硬盘
							...
							succfn: function(data){
								if(data && data.content && data.content.echartsData && data.content.echartsData.line1 && data.content.echartsData.line1.length){
									var eData = data.content.echartsData;
									$('#' + disk, $el).find('.aixDetails-diskIO').text(eData.line3[eData.line3.length - 1] + '%');
									$('#' + disk, $el).find('.aixDetails-diskIn').text(eData.line1[eData.line1.length - 1] + 'kbps');
									$('#' + disk, $el).find('.aixDetails-diskOut').text(eData.line2[eData.line2.length - 1] + 'kbps');
								}
							}
						});
						dickEchart.start();
