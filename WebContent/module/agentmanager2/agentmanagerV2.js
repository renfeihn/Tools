	...
	return {
		load: function($el, scope, handler) {
			...
			this.delegateEvents({
				...
				'click #alertBtn':function() {
					var $p = $('#agmTbody',$el).find('input:checked').parents('tr');
					var name = $p.find('td:eq(1)').text();
					var ip = $p.find('td:eq(3)').text();
				
					$('.modal-info-name',$el).text(name);
					$('.modal-info-ip',$el).text(ip);
					$('#alertModal',$el).modal('show');
				}

			})
			
			var alertRuleTab = $('#alertRuleTab',$el).DataTable({
				"stateSave": false,
				"autoWidth": true,
				"ordering": true,
				"searching" : false,
				...
				"columns":[{
					...
					data:'rule',defaultContent:'-'
				}]
			});
			var tabData = [{
				index:1,desc:'持续10分钟无日志',type:'持续时间',time:'5分钟',rule:'流量小于5K'
			},{
				index:2,desc:'12:00-13:00无日志',type:'固定时间间隔',time:'12:00-13:00',rule:'流量大于500M'
			},{
				index:3,desc:'持续30分钟无日志',type:'持续时间',time:'30分钟',rule:'流量小于2M'
			}];
			alertRuleTab.rows.add(tabData).draw();
