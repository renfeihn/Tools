...
! function($) {
	...
	Collapse.prototype = {
		...
		show: function() {
				...
				//2015/08/11 9:54
				//如果是asideMenu则不会自动收起
				if(this.$parent && !this.$parent.hasClass('aside-menu')) {
					...
					actives = this.$parent && this.$parent.find('> .accordion-group > .in');
					var timeout = .35;
					this.$element.find('.nav-list').children().each(function(index, elem) {
		...
		reset: function(size) {
				...
				// 展开手风琴时滚动到当前部位
				var $aside;
				if(($aside = this.$element.closest('#leftAside')).length) {
...
! function($) {
	...
	Modal.prototype = {
		...
		show: function() {
				...
				//  将遮罩层以及提示栏的z-index提高比模态框（1050）更高的1052，可以让在使用模态框的同时，正常使用遮罩以及提示栏。
				app.shelter.upperZIndex(false,false,false,this.options.keyboard);
				...
				//增加拖动功能,该功能依赖于jqueryUI
				if(this.$element.draggable && typeof this.$element.draggable == "function") {
					...
					var halfWidth = parseInt(this.$element.outerWidth()) / 2;
					this.$element.addClass("height_important_auto").css({ "left": "calc(50% - " + halfWidth + "px)","top":"110px", "right": "auto", "transition": "none","height":"auto" });
				}
				//拖动功能end
		...
		hide: function(e) {
				...
				//清除拖动功能                        
				if(this.$element.draggable && typeof this.$element.draggable == "function"&&this.hasInitDrag==true) {
					this.$element.draggable("destroy");
					this.$element.css({"transition": "all 0.5s" });
					this.hasInitDrag=false;
				}
				//清除拖动功能end    
