	...
	function(undefined) {
		...
		(function(factory) {
			...
			if ( typeof define === "function" && define.amd) {
				define(["jquery", "underscore", "backbone", "handler2"], factory);
			}
			// global
			else {
				factory();
			...
			Dispatcher2.prototype = {

				constuctor : Dispatcher2,
				...
				load : function(opt, moduleId, section, subSection) {
					...
					require(["script/lib/text!./" + mvvmConfPath + "?" + new Date().getTime()], function(mvvmConf) {
						if (opt.frameRenderTo) {
							if(typeof opt.frameRenderTo == 'string'){
								var tmpFrameRenderTo = opt.frameRenderTo;
								if(!/^#+.*/.test(opt.frameRenderTo)){
									tmpFrameRenderTo = '#'+tmpFrameRenderTo;
								}
								handler = new Handler2(mvvmConf, modulePath, tmpFrameRenderTo, $.extend(opt.params, params), opt.context, title);
								if(opt.id != undefined){
									handler.cacheId = opt.id;
								}else{
									handler.cacheId = opt.frameRenderTo;
								}
							}else{
								handler = new Handler2(mvvmConf, modulePath, opt.frameRenderTo, $.extend(opt.params, params), opt.context, title);
								handler.cacheId = opt.id;
							}
						} else {
							...
							try {
								if (containerId) {
									...
									// 控制页面流转
									// 维护页面template生命周期
									handler = new Handler2(mvvmConf, modulePath, "#" + containerId);
									handler.cacheId = containerId;
								} else {
									return;
								}
							} catch (e) {
								handler = new Handler2(dispatcher.mvvm404, dispatcher.modulePath404, "#" + containerId);
								handler.stepTo(0);
								handler.cacheId = containerId;
							}
						}
