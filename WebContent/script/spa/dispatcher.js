...
( /*<global>*/ function (undefined) {
	...
	(function (factory) {
		...
		if (typeof define === "function" && define.amd) {
			define(["jquery", "underscore", "backbone", "handler"], factory);
		}
		//global
		else {
			factory();
		...
		var Dispatcher = function () {
			...
			this.frameRenderTo = "#awebPageFrame";
		...
		Dispatcher.prototype = {
             ...
             * @param  {[string]} id []
			 */
            /**两种接口，第一种是（标题，模块ID[，section[,子section]]）
             *第二种接口，（options{
             * title:标题， String
             * moduleId:模块ID, String
             * section String or Array
             *  }）
             */


			load: function (opt,moduleId,section,subSection) {
                var title,tabId,sections=[],
				...
				//TODO 为在页面引入多个模块js去掉时间
//				require(["script/lib/text!./" + mvvmConfPath + "?" + new Date().getTime()], function (mvvmConf) {
				require(["script/lib/text!./" + mvvmConfPath], function (mvvmConf) {
					if(opt.frameRenderTo){
                        handler = new Handler(mvvmConf, modulePath, opt.frameRenderTo, opt.params,title);
                        ...
                        try {
                            if(containerId){
                                ...
                                //	控制页面流转
                                //	维护页面template生命周期
                                handler = new Handler(mvvmConf, modulePath, "#" + containerId, opt.params,title);
                                handler.cacheId = containerId;
                            }else{
                                return;
                            }
                        } catch (e) {
                            handler = new Handler(dispatcher.mvvm404, dispatcher.modulePath404, "#" + containerId);
                            handler.stepTo(0);
                            handler.cacheId = containerId;
                        }
                    }
			...
			pause: function () {
				...
				&& (this.cache[this.currentHandler.cacheId] = this.currentHandler);
			},
