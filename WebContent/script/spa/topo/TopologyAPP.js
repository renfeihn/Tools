 ...
 * topo图插件 d3 v4.x
 */
define(['d3V5', 'TopoDragType', 'Invented', 'Menu', 'AuxLine', 'CreateTemplateRect','script/spa/topo/common/colorMap'], 
		function (d3, TopoDragType, Invented, Menu, AuxLine, CreateTemplateRect, colorMap) {
	let Topology = function  (ele, opts = {}) {
		...
		this.NotOperateLine = opts.NotOperateLine || false;
	...
	Topology.prototype = {
		...
		mouseoverNode (d) {
			...
			up.links.forEach(item => {
				this.svg.select('path#line_'+item.id).attr('class', 'activeSelectLineHover')
			})
			up.links.forEach(item => {
				this.svg.select('rect#rect'+item.index).attr('class', 'edge-label-relation activeSelectLineHover')
			})
		...
		setCenterNode () {
			...
			this.nodes = [].concat(tmp, topTmp, bottomTmp);
			this.nodes = this.nodes.map(item => {
				item.x = this.dragType.stepRule(item.x);
				item.y = this.dragType.stepRule(item.y);
				item.fx = item.x;
				item.fy = item.y;
				return item;
			})
		...
		setTopCenterNode () {
			let tmp = this.nodes.filter(item => {
				if (item.type === '0') {
					item.y = this.h / 2 - this.linkDistance;
					return true
				}
				return false;
			})
			...
			if (tmp.length === 1) {
				...
				for(var i = tmp.length - 1 ; i >= tmp.length / 2 ; i --) {
					tmp[i].x = this.w / 2 + startX * (i + 1)
				}
			} else {
				...
				for(var i = (tmp.length - 2 ) ; i >= (tmp.length - 1) / 2 ; i --) {
					tmp[i].x = this.w / 2 + startX * (tmp.length - i - 1)
				}
				tmp[tmp.length - 1].x = this.w / 2;
			}
			return tmp;
		},
		setBottomCenterNode () {
		...
		removeLink (source, target, d) {
			...
			this.links = links;
			this.update();
			this.opts.removeRelationModal && this.opts.removeRelationModal(d.source.dmDtId,d.target.dmDtId);
		},
		removeNode (id) {
		...
		addLinks (links) {
			...
			var tmpLink = links.map(item => {
				if (!item.relId) {
					if (item.source.id) {
						item.id = item.source.id +'_'+ item.target.id
					}else {
						item.id = item.source +'_'+ item.target
					}
				} else {
					if (item.source.id) {
						item.id = item.relId + item.source.id +'_'+ item.target.id
					}else {
						item.id = item.relId + item.source +'_'+ item.target
					}
				}
				return item;
			})
			tmpLink.forEach(link => {
			...
			var sameLinks = this.unique(this.links);
			
			sameLinks.forEach(item => {
				var s = item.source;
				var t = item.target;
		...
		unique (links) {
			...
			for(var i = 0 ; i < links.length; i ++ ) {
				for(var j = 1; j < links.length ; j ++ ) {
					var sourceId1 = links[i].source.id ? links[i].source.id: links[i].source;
					var targetId1 = links[i].target.id ? links[i].target.id: links[i].target;
					var sourceId2 = links[j].source.id ? links[j].source.id: links[j].source;
					var targetId2 = links[j].target.id ? links[j].target.id: links[j].target;
					if (sourceId1 === targetId2 && targetId1 === sourceId2) {
						if (same.length === 0) {
							same.push({
								source: links[i],
								target: links[j]
							...
							if (!same.some(function (item) {return item.source.relId === links[i].relId})) {
								same.push({
									source: links[i],
									target: links[j]
								});
		...
		triggerZoom (zoom) {
			...
			this.scale = zoom;
			this.svg.select('g.outerG').attr('transform', `scale(${zoom})`)
		},
		restoreZoom () {
			this.scale = 1;
			this.svg.select('g.outerG').attr('transform', null)
		},
		getSelectNode () {
		...
		setPosTopCenter (){
			return {
				x: this.dragType.stepRule(this.w / 2 - 40),
				y: 40
			}
		},
		setPosCenter (){
			return {
				x: this.dragType.stepRule(this.w / 2 - 40),
				y: this.dragType.stepRule(this.h / 2 - 40),
			}
		}
	}
