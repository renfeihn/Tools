...
(function (factory) {
	 ...
	 */
	var Handler = function (module, modulePath, renderTo, params, title) {
		//此处简单校验配置逻辑
		if(module){
			try{
		...
		return new Handler.fn.init(module, modulePath, renderTo, params,title);
	};
	
	Handler.AsyncEvent = function (func, time) {
		this.func = func;
		this.time = time;
		this.windowId = 0;
		this.uniqueId = 0;
	...
	Handler.fn = Handler.prototype = {
		 ...
		 * @param {[string]} modulePath [模块路径]
		 */
		init: function (module, modulePath, renderTo, params, title) {
		 ...
		 */
		pause: function () {
			var that = app.dispatcher.cache[this.cacheId];
			if (that) 
				that = null;

			app.dispatcher.cache[this.cacheId] = this;
		...
		setTimeout: function(func, timeout) {
			var handler = this, 
				event = new Handler.AsyncEvent(func, timeout), 
		...
		clearTimeout: function (uniqueId) {
            ...
            if(event) {
                window.clearTimeout(event.windowId);
                this.removeAsyncEvent(this.timeouts, event.uniqueId);
            }
        },
		
		setInterval: function (func, timespan) {
            var event = new Handler.AsyncEvent(func, timespan),
		...
		clearInterval: function (uniqueId) {
            ...
            if(event){
                window.clearInterval(event.windowId);
		...
		stopAsyncEvent: function () {
			for (var i in this.intervals) 
				window.clearInterval(this.intervals[i].windowId);
			
			for (var i in this.timeouts) 
				window.clearTimeout(this.timeouts[i].windowId);
		}, 
		...
		startAsyncEvent: function () {
			...
			for (var i in this.timeouts) {
				...
				event.windowId = window.setTimeout(function () {
                    event.func();
