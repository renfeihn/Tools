...
$.fn.dataStatistics = function(options){
	...
	function run(){
		...
		function increase() {
			//执行次数为0时,停止执行
			if(options.alertNumber&&options.alertCallback){
				var mileDiffere = options.max - options.alertNumber ;
				if(mileDiffere == difference){
					options.alertCallback&&options.alertCallback();
				}
			}
			if (difference<1) {
